...
@Component({
  ...
  template: `
    <section scrollAnimate animateVariant="fade-up" class="app-shell-panel space-y-5 p-5 lg:p-6">
      ...
      <form class="rounded-[20px] border border-base-300/70 bg-base-100/70 p-4 shadow-lg" (ngSubmit)="addCategory()">
        <div class="flex gap-3">
          <input [(ngModel)]="newCategoryName" name="name" placeholder="New Category Name" class="app-input flex-1" required />
          <button type="submit" class="btn btn-primary" [disabled]="!newCategoryName">Add</button>
        </div>
      </form>
        ...
        <table class="table table-zebra table-pin-rows">
          ...
          <tbody>
            @for (item of categories; track item.id) {
              ...
              <tr><td colspan="3" class="py-6 text-center text-base-content/60">No categories found.</td></tr>
            }
          </tbody>
        </table>
      </div>
    ...
    <dialog id="category-delete-modal" class="modal modal-bottom sm:modal-middle">
...
export class CategoriesComponent implements OnInit {
  ...
  openDeleteModal(id: string, name: string) {
    ...
    const modal = document.getElementById('category-delete-modal') as HTMLDialogElement;
