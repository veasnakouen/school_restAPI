...
public sealed class StudentAssessmentRequestReportService : IStudentAssessmentRequestReportService
{
    public async Task<byte[]> GeneratePdfAsync(StudentAssessmentRequestFormDto request, CancellationToken cancellationToken = default)
    {
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Column(column =>
                {
                    ...
                    column.Item().Row(row =>
                    {
                        row.ConstantItem(78).Height(60).AlignMiddle().AlignCenter().Border(1).BorderColor(Colors.Grey.Lighten2).Background(Colors.Blue.Lighten4)
                            .Text("LOGO")
                            .FontSize(16)
                            .Bold()
                        ...
                        {
                            header.Item().AlignCenter().Text("Student Assessment Form").FontSize(18).Bold();
                            header.Item().AlignCenter().Text("Assessment Request for Student Equipment or Support").FontSize(11).FontColor(Colors.Grey.Darken2);
                        });
                    });
                    ...
                    column.Item().Row(row =>
                    {
                        row.RelativeItem().Column(left =>
                        {
                            ...
                            left.Item().Text($"Student Name: {request.StudentName}");
                            left.Item().Text($"Class Name: {request.ClassName}");
                            left.Item().Text($"Gender: {request.Gender}");
                        });

                        row.RelativeItem().Column(right =>
                        {
                            right.Spacing(4);
                            right.Item().Text($"Date of Birth: {(request.DateOfBirth.HasValue ? request.DateOfBirth.Value.ToString("dd-MM-yyyy") : string.Empty)}");
                            right.Item().Text($"Request By: {request.RequestBy}");
                            right.Item().Text($"Department: {request.DepartmentName}");
                            right.Item().Text($"Checked By: {request.CheckedBy}");
                        });
                    });
    ...
    public Task<byte[]> GenerateExcelAsync(StudentAssessmentRequestFormDto request, CancellationToken cancellationToken = default)
    {
        using var package = new ExcelPackage();
        var worksheet = package.Workbook.Worksheets.Add("Student Assessment");

        worksheet.Cells["A1:H1"].Merge = true;
        worksheet.Cells["A1"].Value = "Student Assessment Form";
        worksheet.Cells["A1"].Style.Font.Bold = true;
        worksheet.Cells["A1"].Style.Font.Size = 16;
        worksheet.Cells["A1"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

        worksheet.Cells["A2:H2"].Merge = true;
        worksheet.Cells["A2"].Value = "Assessment Request for Student Equipment or Support";
        ...
        worksheet.Cells["C4"].Value = $"Student Name: {request.StudentName}";
        ...
        worksheet.Cells["G4"].Value = $"Class Name: {request.ClassName}";

        worksheet.Cells["A5:B5"].Merge = true;
        worksheet.Cells["A5"].Value = $"Gender: {request.Gender}";
        worksheet.Cells["C5:D5"].Merge = true;
        worksheet.Cells["C5"].Value = $"Date of Birth: {(request.DateOfBirth.HasValue ? request.DateOfBirth.Value.ToString("dd-MM-yyyy") : string.Empty)}";
        worksheet.Cells["E5:F5"].Merge = true;
        worksheet.Cells["E5"].Value = $"Request By: {request.RequestBy}";
        worksheet.Cells["G5:H5"].Merge = true;
        worksheet.Cells["G5"].Value = $"Department: {request.DepartmentName}";
